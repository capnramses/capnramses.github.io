...
function drawScene (gl) {
  ...
  //alert ("drew plane");
	...
	if (!pauseRendering) {
		...
		gl.enableVertexAttribArray (vnAttribute);
		gl.enableVertexAttribArray (happyAttrib);
		gl.enableVertexAttribArray (madAttribLoc);
		// bind texture
		...
		gl.vertexAttribPointer (vertexPositionAttribute, 3, gl.FLOAT, false, 0, 0);  // tell shader attrib where to find data
		gl.bindBuffer (gl.ARRAY_BUFFER, vboVTHandle);
		gl.vertexAttribPointer (textureCoordAttribute, 2, gl.FLOAT, false, 0, 0); 
		gl.bindBuffer (gl.ARRAY_BUFFER, vboVNHandle);
		gl.vertexAttribPointer (vnAttribute, 3, gl.FLOAT, false, 0, 0); // link shader attribute to VBO
		gl.bindBuffer (gl.ARRAY_BUFFER, happyVBO);
		gl.vertexAttribPointer (happyAttrib, 3, gl.FLOAT, false, 0, 0);
		gl.bindBuffer (gl.ARRAY_BUFFER, madVBO);
		gl.vertexAttribPointer (madAttribLoc, 3, gl.FLOAT, false, 0, 0);
		// draw call
		...
		gl.disableVertexAttribArray (vnAttribute);
		gl.disableVertexAttribArray (happyAttrib);
		gl.disableVertexAttribArray (madAttribLoc);
	}
}
