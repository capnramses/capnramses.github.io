...
var g_fb_half_vs_url = "shaders/fb_pt.vert";
var g_fb_half_fs_url = "shaders/fb_pt.frag";
var g_fb_half_sp;
var g_fb_half_blur_vs_url = "shaders/fb_blur_pt.vert";
var g_fb_half_blur_fs_url = "shaders/fb_blur_pt.frag";
var g_fb_half_blur_sp;
var g_fb_half;
var g_fb_half_rgba_tex;
var g_fb_half_blur;
var g_fb_half_blur_rgba_tex;
...
function main () {
	...
	g_fb_sp_half_rgba_tex_loc = get_uniform_loc (g_fb_sp, "fb_half_rgba_tex");
	...
	gl.uniform1i (g_fb_sp_half_rgba_tex_loc, 1);
	gl.uniform1i (g_fb_sp_d_tex_loc, 2);
	
	g_fb_half_sp = load_shaders_from_files (g_fb_half_vs_url, g_fb_half_fs_url);
	gl.linkProgram (g_fb_half_sp);
	
	g_fb_half_blur_sp = load_shaders_from_files (g_fb_half_blur_vs_url, g_fb_half_blur_fs_url);
	gl.linkProgram (g_fb_half_blur_sp);
	...
	g_fb_half_rgba_tex = gl.createTexture();
	gl.bindTexture(gl.TEXTURE_2D, g_fb_half_rgba_tex);
	// linear filter to support SSAA
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
	...
	//gl.texImage2D (gl.TEXTURE_2D, 0, g_srgb_ext.SRGB_ALPHA_EXT, gl_width * g_ssf,
	//	gl_height * g_ssf, 0, g_srgb_ext.SRGB_ALPHA_EXT, gl.UNSIGNED_BYTE, null);
	gl.texImage2D (gl.TEXTURE_2D, 0, gl.RGBA, gl_width * g_ssf * 0.5,
		gl_height * g_ssf * 0.5, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
	
	g_fb_half_blur_rgba_tex = gl.createTexture();
	gl.bindTexture(gl.TEXTURE_2D, g_fb_half_blur_rgba_tex);
	// linear filter to support SSAA
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
	gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
	...
	g_fb_half = gl.createFramebuffer ();
	gl.bindFramebuffer (gl.FRAMEBUFFER, g_fb_half);
	gl.framebufferTexture2D (gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D,
		g_fb_half_rgba_tex, 0);
	
	g_fb_half_blur = gl.createFramebuffer ();
	gl.bindFramebuffer (gl.FRAMEBUFFER, g_fb_half_blur);
	gl.framebufferTexture2D (gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D,
		g_fb_half_blur_rgba_tex, 0);
...
function main_loop () {
	...
	gl.flush ();
	gl.finish ();
	gl.viewport (0, 0, gl_width / 2, gl_height / 2);
	gl.bindFramebuffer (gl.FRAMEBUFFER, g_fb_half);
	gl.useProgram (g_fb_half_sp);
	...
	//
	// blur the half size one
	gl.flush ();
	gl.finish ();
	gl.viewport (0, 0, gl_width / 2, gl_height / 2);
	gl.bindFramebuffer (gl.FRAMEBUFFER, g_fb_half_blur);
	gl.useProgram (g_fb_half_blur_sp);
	gl.activeTexture (gl.TEXTURE0);
	gl.bindTexture (gl.TEXTURE_2D, g_fb_half_rgba_tex);
	g_vao_ext.bindVertexArrayOES (g_fb_vao);
	gl.drawArrays (gl.TRIANGLE_STRIP, 0, 4);
