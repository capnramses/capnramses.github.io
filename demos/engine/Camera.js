...
function Camera (fov_deg, aspect, nearClip, farClip, wc_pos, direction) {
	...
	this.mViewMat = mat4.lookAt (wc_pos, targ, [0, 1, 0], this.mViewMat);
	...
	this.setPos = function (wc_pos) {
		this.mWC_Pos = wc_pos;
		document.getElementById('campos').innerHTML="<small>(" + this.mWC_Pos[0].toFixed(2) + ", " + this.mWC_Pos[1].toFixed(2) + ", " + this.mWC_Pos[2].toFixed(2) + ")</small>";
		var targ = [0, 0, 0];
		targ[0] = wc_pos[0] + this.mDirection[0];
		targ[1] = wc_pos[1] + this.mDirection[1];
		targ[2] = wc_pos[2] + this.mDirection[2];
		document.getElementById('camdir').innerHTML="<small>(" + this.mDirection[0].toFixed(2) + ", " + this.mDirection[1].toFixed(2) + ", " + this.mDirection[2].toFixed(2) + ")</small>";
