...
function ShaderProgramme (gl) {
	...
	this.loadVertexShaderFromURL = function (gl, url, popform) {
		...
		return this.loadVertexShader (gl, sourceString);
	}
	
	// fetch from url, load, compile, and attach a fragment shader
	this.loadFragmentShaderFromURL = function (gl, url, popform) {
		var sourceString = getURLAsString (url);
		...
		return this.loadFragmentShader (gl, sourceString);
	}
	...
	this.setUniformIntByLocation = function (gl, location, value) {
		gl.uniform1i (location, value);
	}
	
	// set a unform variable value based on its location (might be a bit faster than looking up the name each time)
	this.setUniformFloatByLocation = function (gl, location, value) {
		gl.uniform1f (location, value);
	}
	
	// set a unform variable value based on its name
	this.setUniformMat4ByName = function (gl, variableName, matrix) {
		var location = this.getUniformLocation(gl, variableName);
