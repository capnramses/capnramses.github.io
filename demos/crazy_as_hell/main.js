...
function main () {
	...
	var bearings_url = "speaker_bearings.json";
	...
	g_bearings = load_bearings (bearings_url);
	g_speakers_prog_idx = load_shaders (svs_url, sfs_url);
	loc = gl.getUniformLocation (g_speakers_prog_idx, "bearing");
...
function render () {
	...
	gl.activeTexture (gl.TEXTURE0);
	gl.drawArrays (gl.POINTS, 0, g_vbos.count);
...
function create_model_mat (step_num) {
	...
	var distance = 2.0;
	var radians = g_bearings[step_num] * 0.0174532925;
	var x = Math.tan (radians) / distance;
	model_mat = mat4.translate (model_mat, [x, 0.2, 0.0], model_mat);
