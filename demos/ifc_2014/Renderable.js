...
function Renderable () {
	...
	this.loadMeshFromJSON = function (url) {
		...
		xmlhttp.send (null);
		this.createVPBuffer (json.vps);
		this.createVTBuffer (json.gs);
		this.createVNBuffer (json.vns);
	}
	...
	this.render = function (shaderprog) {
		...
		if (shaderprog.mHasVTAttrib) {
			...
			gl.vertexAttribPointer (shaderprog.mVTAttribLoc, 1, gl.FLOAT, false, 0, 0); // tell current shader how to get memory from buffer
		}
